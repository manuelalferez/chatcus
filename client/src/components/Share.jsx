...
const SHARE_OPTIONS = [
  {
    ...
    url: (url, text) => `https://www.linkedin.com/shareArticle?mini=true&title=${text}&source=Chatcus&url=${url}`,
  },
  {
    type: 'Facebook',
...
export const ShareButton = ({ link, text }) => {
  ...
  const getDropdownData = () => ({
    label: (
      <svg className="ml-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30" width="30px" height="30px">
      ...
      <div className="py-1" role="none">
        ...
        {SHARE_OPTIONS.map((option, i) => (
          <a
            ...
            className="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100 text-gray-900 flex items-center justify-between"
            role="menuitem"
            tabIndex={dropdownState ? 0 : -1}
            id={`menu-item-${i + 1}`}
            target="_blank"
            rel="noreferrer"
          >
            <span>Share to {option.type}</span>
            <svg
